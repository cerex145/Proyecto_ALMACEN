...
async function main() {
  ...
  // Group demands and rows by Product + Lote
  const aggregatedDemands = {}; // key: product|lote => { product, lote, qtyRequested, rows: [] }
  ...
  // Evaluate each aggregated product + lote demand against actual stock
  ...
  let mdTable = `| Código de Producto | Descripción / Nombre | Lote | Stock Inicial en BD | Total Pedido en Excel | Total Faltante | Filas del Excel (Celdas) |\n`;
  mdTable += `| :--- | :--- | :---: | :---: | :---: | :---: | :--- |\n`;

  outputList.forEach(item => {
    // Truncate celdas if extremely long, but keep a readable list
  ...
  fs.writeFileSync(path.join(__dirname, 'miret_collapsed_table.md'), mdTable, 'utf8');
  console.log('Tabla resumida escrita en miret_collapsed_table.md');
