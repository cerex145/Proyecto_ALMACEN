/**
 * MIGRACIÓN CORRECTA - CON MAPEO DE PRODUCTOS
 */
...
console.log('\n' + '═'.repeat(70));
console.log('🚀 MIGRACIÓN CORRECTA - CON MAPEO DE PRODUCTOS');
...
async function migrar() {
    ...
    try {
        ...
        const [productsRows] = await conn.execute('SELECT id, codigo FROM productos');
        const productoMap = new Map();
        productsRows.forEach(p => {
            if (p.codigo) productoMap.set(p.codigo.trim().toUpperCase(), p.id);
        });
        console.log(`✅ ${productoMap.size} productos cargados\n`);
        ...
        for (let i = 1; i < ingLineas.length; i++) {
            ...
            const codProd = (v[1] || '').trim().toUpperCase();
            const lote = (v[3] || '').trim();
            ...
            // Buscar producto
            const productoId = productoMap.get(codProd);
            if (!productoId) {
                if (ingSkipped === 0) console.log(`⚠️  Línea ${i}: Producto ${codProd} no encontrado, saltando...`);
                ingSkipped++;
                continue;
            }
            ...
            try {
                ...
                await conn.execute(
                    `INSERT INTO nota_ingreso_detalles (nota_ingreso_id, producto_id, lote_numero, cantidad) 
                     VALUES (?, ?, ?, ?)`,
                    [r.insertId, productoId, lote, cantidad]
                );
        ...
        if (ingSkipped > 0) console.log(`⚠️  Total productos no encontrados: ${ingSkipped}\n`);
        ...
        for (let i = 1; i < salLineas.length; i++) {
            ...
            const productoId = productoMap.get(codProd);
            if (!productoId) {
                if (salSkipped === 0) console.log(`⚠️  Línea ${i}: Producto ${codProd} no encontrado, saltando...`);
                salSkipped++;
                continue;
            }
            ...
            try {
                ...
                await conn.execute(
                    `INSERT INTO nota_salida_detalles (nota_salida_id, producto_id, cantidad) 
                     VALUES (?, ?, ?)`,
                    [r.insertId, productoId, cantidad]
                );
        ...
        if (salSkipped > 0) console.log(`⚠️  Total productos no encontrados: ${salSkipped}\n`);
