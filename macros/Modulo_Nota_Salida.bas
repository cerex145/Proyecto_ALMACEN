Attribute VB_Name = "Modulo_Nota_Salida"
'MODULO SALIDA

Public Sub ActualizarListasNotaSalida()
    With Sheets("Nota de Salida")
        ' Cargar RUC
        CargarListaUnica .OLEObjects("cmb12"), Sheets("Registro Productos").Range("I22:I10000")
        
        ' Cargar T. Documento
        CargarListaUnica .OLEObjects("cmb18"), Sheets("Registro Productos").Range("O22:O10000")
    End With
End Sub
...
Public Sub FiltrarCodigoPorRUC(rucSeleccionado As String)
    ...
    With Sheets("Nota de Salida").OLEObjects("cmb13").Object
        .Clear
...
Public Sub FiltrarLotePorCodigo(rucSeleccionado As String, codigoClienteSeleccionado As String)
    ...
    With Sheets("Nota de Salida").OLEObjects("cmb14").Object
        .Clear
        For Each item In listaFiltrada
            .AddItem item
        Next item
...
Public Sub FiltrarProductoPorLote(rucSeleccionado As String, codigoClienteSeleccionado As String, loteSeleccionado As String)
    ...
    With Sheets("Nota de Salida").OLEObjects("cmb15").Object
        .Clear
        For Each item In listaFiltrada
            .AddItem item
        Next item
...
Public Sub CompletarInfoProducto(productoSeleccionado As String)
    ...
    Dim cBulto As Variant, cCajas As Variant, cCaja As Variant, cFraccion As Variant, cTotal As Variant
    Dim rucs As Variant, codigos As Variant, lotes As Variant
    ...
    With Sheets("Nota de Salida")
        rucSeleccionado = NormalizarTexto(.OLEObjects("cmb12").Object.Value)
        codigoSeleccionado = NormalizarTexto(.OLEObjects("cmb13").Object.Value)
        loteSeleccionado = NormalizarTexto(.OLEObjects("cmb14").Object.Value)
        productoSeleccionado = NormalizarTexto(.OLEObjects("cmb15").Object.Value)
    End With
        ...
        If NormalizarTexto(productos(i, 1)) = productoSeleccionado And _
           ...
           NormalizarTexto(lotes(i, 1)) = loteSeleccionado Then
            ...
            With Sheets("Nota de Salida")
                ...
                .Range("J6").Value = fVcto(i, 1)
                .Range("K6").Value = um(i, 1)
                .Range("L6").Value = cBulto(i, 1)
                .Range("M6").Value = cCajas(i, 1)
                .Range("N6").Value = cCaja(i, 1)
                .Range("O6").Value = cFraccion(i, 1)
                .Range("P6").Value = cTotal(i, 1)
            End With
            Exit For
        End If
    Next i
...
Public Sub FiltrarNumeroPorDocumentoSalida(tipoDocSeleccionado As String)
    Dim hoja As Worksheet
    ...
    With Sheets("Nota de Salida").OLEObjects("cmb19").Object
        .Clear
        For Each item In listaFiltrada
            .AddItem item
        Next item
...
Public Sub FiltrarFechaPorDocumento(tipoDocSeleccionado As String, nroDocSeleccionado As String)
    ...
    With Sheets("Nota de Salida").OLEObjects("cmb21").Object
        .Clear
        For Each item In listaFiltrada
