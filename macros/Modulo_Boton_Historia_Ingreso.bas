Attribute VB_Name = "Modulo_Boton_Historia_Ingreso"
Sub Elimimnar_Historia_Ingreso()
    ...
    If TypeName(Selection) <> "Range" Then
        MsgBox "Por favor, selecciona una o más celdas dentro del rango permitido (D5:Q...)", vbExclamation
        Exit Sub
    End If
    ...
    For Each celda In Selection.Cells
        ...
        ' Validar rango: fila > 4 y columnas D (4) a P (17)
        If fila > 4 And columna >= 4 And columna <= 17 Then
            clave = CStr(fila)
    ...
    If filasAfectadas.Count = 0 Then
        MsgBox "No se seleccionaron celdas dentro del rango válido (columnas D a Q, filas mayores a 4).", vbExclamation
        Exit Sub
    End If
    ...
    For i = filasAfectadas.Count To 1 Step -1
        ...
        Range(Cells(fila, 4), Cells(fila, 17)).ClearContents
    Next i
